...
`yarn audit:chain:evidence` runs the nine fixed security-audit sink, hash-chain and encrypted-writer tests with the pinned workspace Go toolchain and writes a schema-v1 local-only report. The report binds the exact command/test set, runtime and source commit, fifteen source files, raw stdout/stderr, process status, limitations and a five-file checksum set. `yarn audit:chain:verify` and the aggregate evidence verifier reject source, scope, contract, output-marker, checksum and extra-artifact tampering. This is application-side repository evidence, not proof of a production audit deployment.
