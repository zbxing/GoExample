...
| `Services/Billing` | GoExample maintainers | Go `1.0.0` | OpenAPI `1.0.0` | Uses canonical `GET /readyz` | Independent Framework `net/http` service exposes `GET /api/v1/billing/summary`; SDK package behavior tests cover all 14 operations, gateway prefixes, request editors, envelope decoding and response limits, while a service-module integration test drives the generated SDK through the real Framework handler | Not available; repository-local service only |

The generated SDK covers all 26 published operation IDs for Example and the smaller 14-operation Billing contract is generated independently. `contracts/projects.json` selects the source document for each managed service; `yarn sdk:check --project <project>` regenerates that service's SDK from its workspace or pinned Git contract, rejects unsupported contract shapes, compares the exact formatted source, and requires the matching SDK `VERSION` to match OpenAPI `info.version`.
