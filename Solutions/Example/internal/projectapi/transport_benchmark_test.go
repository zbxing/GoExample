...
import (
	...
	"github.com/zbxing/goexample/Solutions/Example/internal/projectapp"
)
...
type transportScenarioWorkload struct {
	Name                string
	Path                string
	Requests            int
	Concurrency         int
...
type transportScenarioMeasurement struct {
	SchemaVersion        int     `json:"schemaVersion"`
	Transport            string  `json:"transport"`
	...
	DependencyDelayNanos int64   `json:"dependencyDelayNanos"`
	PayloadBytes         int64   `json:"payloadBytes"`
	ThroughputRPS        float64 `json:"throughputRps"`
	P50Nanos             int64   `json:"p50Nanos"`
	P95Nanos             int64   `json:"p95Nanos"`
...
func TestProjectTransportCapacityMatrixTCP(t *testing.T) {
	...
	workloads := []transportCapacityWorkload{
		...
		{Name: "steady-c2", Requests: 600, Concurrency: 2},
		{Name: "steady-c4", Requests: 800, Concurrency: 4},
		{Name: "steady-c8", Requests: 1200, Concurrency: 8},
		{Name: "steady-c16", Requests: 2000, Concurrency: 16},
		{Name: "steady-c32", Requests: 3000, Concurrency: 32},
		{Name: "steady-c64", Requests: 4000, Concurrency: 64},
		{Name: "steady-c128", Requests: 6000, Concurrency: 128},
...
func TestProjectTransportScenarioMatrixTCP(t *testing.T) {
	if runtime.GOOS != "linux" {
		t.Skip("comparable transport scenario measurements run only on the fixed Linux target")
	}

	const invalidBearer = "Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJiZW5jaG1hcmsifQ.invalid"
	scenarios := []transportScenarioWorkload{
		{
			Name:                "response-32k-c16",
			Path:                "/api/v1/benchmark/payload",
			Requests:            800,
			Concurrency:         16,
		...
		},
		{
			...
			Path:           "/api/v1/benchmark/protected",
			Requests:       1200,
			Concurrency:    16,
		...
		},
		{
			...
			Path:            "/api/v1/benchmark/dependency",
			Requests:        800,
			Concurrency:     32,
			ExpectedStatus:  http.StatusOK,
			DependencyDelay: 5 * time.Millisecond,
		},
	}
	transports := []struct {
		name  string
	...
	for _, scenario := range scenarios {
		for _, candidate := range transports {
			t.Run(scenario.Name+"/"+candidate.name, func(t *testing.T) {
				var protectedHandlerCalls atomic.Int64
				app := newTransportScenarioApplication(&protectedHandlerCalls)
				server := candidate.start(t, app)
				client := newTransportClient(t)
				payloadBytes, _, err := requestTransportScenario(client, server.baseURL, scenario)
				if err != nil {
					t.Fatalf("measure transport scenario payload: %v", err)
				}
				if payloadBytes < scenario.MinimumPayloadBytes {
					t.Fatalf("scenario payload bytes = %d, want at least %d", payloadBytes, scenario.MinimumPayloadBytes)
				}
				for range 25 {
					bytes, _, err := requestTransportScenario(client, server.baseURL, scenario)
					if err != nil {
						t.Fatalf("warm transport scenario: %v", err)
					}
					if bytes != payloadBytes {
						t.Fatalf("warm scenario payload bytes = %d, want %d", bytes, payloadBytes)
					}
				}

				jobs := make(chan int, scenario.Requests)
				for index := range scenario.Requests {
					jobs <- index
				}
				close(jobs)
				...
				var workers sync.WaitGroup
				started := time.Now()
				for range scenario.Concurrency {
					workers.Add(1)
					go func() {
						defer workers.Done()
						for index := range jobs {
							...
							latencies[index] = time.Since(requestStarted)
							if err == nil && bytes != payloadBytes {
								err = fmt.Errorf("scenario payload bytes = %d, want %d", bytes, payloadBytes)
							}
							if err != nil {
								errorCount.Add(1)
								select {
				...
				failures := errorCount.Load()
				if failures > 0 {
					t.Fatalf("%d scenario requests failed; first error: %v", failures, <-firstError)
				}
				if scenario.Name == "auth-reject-c16" && protectedHandlerCalls.Load() != 0 {
					t.Fatalf("protected handler calls = %d, want 0", protectedHandlerCalls.Load())
				}

				sort.Slice(latencies, func(i, j int) bool { return latencies[i] < latencies[j] })
				measurement := transportScenarioMeasurement{
					SchemaVersion:        1,
					Transport:            candidate.name,
					...
					DependencyDelayNanos: scenario.DependencyDelay.Nanoseconds(),
					PayloadBytes:         payloadBytes,
					ThroughputRPS:        float64(scenario.Requests) / elapsed.Seconds(),
					P50Nanos:             percentileNanos(latencies, 50),
					P95Nanos:             percentileNanos(latencies, 95),
					P99Nanos:             percentileNanos(latencies, 99),
					ErrorCount:           failures,
					ErrorRate:            float64(failures) / float64(scenario.Requests),
				}
				encoded, err := json.Marshal(measurement)
				if err != nil {
					t.Fatalf("encode scenario measurement: %v", err)
				}
				t.Logf("TRANSPORT_SCENARIO %s", encoded)
			})
		}
	}
}
...
func newTransportScenarioApplication(protectedHandlerCalls *atomic.Int64) *fiber.App {
	...
	authService := auth.NewService(auth.Config{
		...
		Issuer:   "transport-benchmark",
		Audience: "transport-benchmark-api",
		TTL:      time.Hour,
		Now:      func() time.Time { return now },
	})
	options := httpapi.Options{
		Name:             "Transport Scenario API",
		...
		Endpoints: []string{
			"GET /api/v1/benchmark/payload",
			"GET /api/v1/benchmark/protected",
			"GET /api/v1/benchmark/dependency",
		},
	}
	options.ApplicationQueries = []httpapi.ApplicationQuery{
		{
			Path: "/benchmark/payload",
			Handler: func(context.Context) (any, error) {
				return map[string]string{"payload": strings.Repeat("x", 32*1024)}, nil
			},
		},
		httpapi.NewAuthenticatedQuery(
			"/benchmark/protected",
			func(context.Context, struct{}, httpapi.ApplicationPrincipal) (map[string]string, error) {
				protectedHandlerCalls.Add(1)
				return map[string]string{"status": "authenticated"}, nil
			},
		),
		{
			Path: "/benchmark/dependency",
...
func startFiberApplicationTransport(tb testing.TB, app *fiber.App) transportServer {
	tb.Helper()
	listener, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		tb.Fatalf("listen for Fiber application transport: %v", err)
	}
...
func startNetHTTPApplicationTransport(tb testing.TB, app *fiber.App) transportServer {
	...
	listener, err := net.Listen("tcp", "127.0.0.1:0")
	if err != nil {
		tb.Fatalf("listen for net/http application transport: %v", err)
	}
	handler, err := httpapi.NewHTTPHandler(app)
	if err != nil {
		listener.Close()
		tb.Fatalf("create net/http application handler: %v", err)
	}
	server := &http.Server{
		Handler:           handler,
	...
	registerServerCleanup(tb, func(ctx context.Context) error {
		return errors.Join(server.Shutdown(ctx), app.ShutdownWithContext(ctx))
	}, serveErrors)
	return transportServer{baseURL: "http://" + listener.Addr().String()}
}
...
func requestTransportScenario(
	client *http.Client,
	baseURL string,
	scenario transportScenarioWorkload,
) (int64, time.Duration, error) {
	request, err := http.NewRequest(http.MethodGet, baseURL+scenario.Path, nil)
	if err != nil {
		return 0, 0, err
	}
	...
	if response.StatusCode != scenario.ExpectedStatus {
		return payloadBytes, connectionWait, fmt.Errorf(
			"status = %d, want %d",
			response.StatusCode,
