...
`Solutions/Example` 是默认示例业务方案服务，module path 为 `github.com/zbxing/goexample/Solutions/Example`。它通过根 `go.work` 和本地 `replace` 使用 [Framework](../../Framework/README.md)，并持有可执行入口、项目路由、环境示例与容器镜像。
...
```powershell
go run ./Solutions/Example/cmd/server
go test ./Framework/... ./Solutions/Example/...
```

服务默认监听 `http://localhost:3001`。

`yarn bench:transports` 的可比测量仅在 Linux 运行：Fiber 与 `net/http` 共享同一 `projectapp.Service` 和真实 TCP 路径，固定执行并发 1/2/4/8/16/32/64/128 的 keep-alive 阶梯及并发 16 的关闭 keep-alive 连接 churn；独立 3 × 2 场景矩阵通过完整 Framework middleware 覆盖至少 32 KiB 响应、无效 Bearer 的 401 拒绝和可取消的 5ms 依赖延迟。5 轮原始结构化数据会被 schema v4 报告器校验并汇总；报告另以“首次达到观测峰值吞吐 90%”定义每个 transport 的经验饱和并发。将上一轮成功报告放入 `.temp/transport-benchmark/baseline.json` 后，报告器会对 9 × 2 容量和 3 × 2 场景结果固定拒绝吞吐下降超过 10%、p95 上升超过 20% 或 p99 上升超过 25%；`environmentFingerprint` 会校验 runner OS/arch/image、CPU、Go、`GOMAXPROCS=2` 及其规范 SHA-256，字段不一致时优先跳过阈值并列出差异。CI 仅从同仓库默认分支成功 push/manual run 自动准备该文件，拒绝 PR 来源并记录候选及环境摘要，无可信候选时保持 `not_checked`。这些固定合成场景不代表目标 payload、目标 IdP、目标依赖、TLS edge 或生产容量证据。
