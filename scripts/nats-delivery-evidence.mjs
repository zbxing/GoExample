...
} from './lib/nats-delivery-evidence.mjs';
...
async function generate() {
  ...
  console.log(
    `NATS delivery evidence written: ${verified.report.status}, ${verified.artifactPaths.length} artifacts`,
  );
...
function verify() {
  const verified = verifyNatsDeliveryEvidence({ repositoryRoot, evidenceRoot });
  console.log(
    `NATS delivery evidence verified: ${verified.report.status}, ${verified.report.goTests.passed.length} Go contract`,
  );
}
...
try {
  ...
  if (extra.length > 0 || !['run', 'verify'].includes(action)) {
    throw new Error('NATS delivery evidence: expected exactly one action: run or verify');
  }
  if (action === 'run') {
    await generate();
  } else {
