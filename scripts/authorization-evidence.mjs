...
import {
  authorizationArtifactNames,
  authorizationGoArguments,
  buildAuthorizationEvidenceReport,
  resolveAuthorizationGoCommand,
  verifyAuthorizationEvidence,
  writeAuthorizationEvidenceChecksums,
} from './lib/authorization-evidence.mjs';
import { isolatedGoToolchainEnvironment } from './lib/go-toolchain-environment.mjs';

const scriptDirectory = path.dirname(fileURLToPath(import.meta.url));
const repositoryRoot = path.resolve(scriptDirectory, '..');
const evidenceRoot = path.join(repositoryRoot, '.temp', 'workflow-artifacts', 'authorization-policy');

function fail(message) {
  console.error(`Authorization evidence: ${message}`);
  process.exit(1);
}

function clearGeneratedArtifacts() {
  mkdirSync(evidenceRoot, { recursive: true });
  for (const name of authorizationArtifactNames) {
...
function run() {
  ...
  const goCommand = resolveAuthorizationGoCommand(repositoryRoot);
  const startedAt = new Date().toISOString();
  const result = spawnSync(goCommand, authorizationGoArguments, {
  ...
  const report = buildAuthorizationEvidenceReport({ repositoryRoot, evidenceRoot, execution });
  ...
  console.log(
    `Authorization evidence ${verified.report.status}: ${verified.artifactPaths.length} checksum-bound artifacts verified`,
  );
...
function verify() {
  ...
  console.log(
    `Authorization evidence verified: ${verified.report.status}, ${verified.artifactPaths.length} artifacts, ${status}`,
  );
}

const [command, ...extra] = process.argv.slice(2);
if (extra.length > 0 || !['run', 'verify'].includes(command)) {
  fail('usage: node scripts/authorization-evidence.mjs <run|verify>');
}
if (command === 'run') {
  run();
} else {
