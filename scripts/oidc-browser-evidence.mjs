...
import {
  ...
  oidcBrowserArtifactNames,
  oidcBrowserGoArguments,
  resolveOIDCBrowserGoCommand,
...
const evidenceRoot = path.join(repositoryRoot, '.temp', 'workflow-artifacts', 'oidc-browser');

function fail(message) {
  console.error(`OIDC browser evidence: ${message}`);
  process.exit(1);
}

function clearGeneratedArtifacts() {
  mkdirSync(evidenceRoot, { recursive: true });
  for (const name of oidcBrowserArtifactNames) {
...
function run() {
  ...
  const goCommand = resolveOIDCBrowserGoCommand(repositoryRoot);
  ...
  const report = buildOIDCBrowserEvidenceReport({ repositoryRoot, evidenceRoot, execution });
  ...
  console.log(
    `OIDC browser evidence ${verified.report.status}: ${verified.artifactPaths.length} checksum-bound artifacts verified`,
  );
...
function verify() {
  ...
  console.log(
    `OIDC browser evidence verified: ${verified.report.status}, ${verified.artifactPaths.length} artifacts, ${status}`,
  );
}

const [command, ...extra] = process.argv.slice(2);
if (extra.length > 0 || !['run', 'verify'].includes(command)) {
  fail('usage: node scripts/oidc-browser-evidence.mjs <run|verify>');
}
if (command === 'run') {
  run();
} else {
