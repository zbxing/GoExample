...
async function generate() {
  ...
  console.log(
    `NATS cluster evidence written: ${verified.report.status}, ${verified.artifactPaths.length} artifacts`,
  );
...
function verify() {
  const verified = verifyNatsClusterEvidence({ repositoryRoot, evidenceRoot });
  console.log(
    `NATS cluster evidence verified: ${verified.report.status}, ${verified.report.goTests.passed.length} Go contract`,
  );
}
...
try {
  ...
  if (extra.length > 0 || !['run', 'verify'].includes(action)) {
    throw new Error('NATS cluster evidence: expected exactly one action: run or verify');
  }
  if (action === 'run') {
    await generate();
  } else {
