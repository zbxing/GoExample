...
`httpclient.Config.Retry` 提供默认关闭的安全方法有限重试。启用时 `MaxAttempts` 包含首次请求且限制为 2–5 次，指数退避最大 5 秒；所有 attempt 共用原始 request context 和 caller/client deadline。仅 GET、HEAD、OPTIONS、TRACE 可重试，带 body 时必须有 `Request.GetBody`；POST、PUT、PATCH、DELETE 即使带幂等键也不会自动重放。无响应 transport 错误以及固定的 408/425/429/502/503/504 可触发重试，中间响应会直接关闭而不无界排空，服务端 `Retry-After` 不会扩张本地退避预算。W3C context 在一个逻辑 CLIENT span 内复用，URL、header、body 和 backend 错误原文不进入 span。该能力不提供熔断、请求去重、写入 exactly-once 或目标环境恢复保证。
