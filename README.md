...
docs/           评估、优化与项目文档
contracts/      Proj 项目与 OpenAPI 分支/tag/commit 的契约清单
go.work         Go 多 module 工作区
```

Framework 与项目工程分离：Framework 提供 HTTP、中间件、健康检查、指标、验证、认证示例和生命周期；`Proj/Example` 持有可执行入口、context-only/typed authenticated project query、`internal/projectapp` application use case、环境配置和镜像。Example 的生产 project route 与 composition root 不导入 Fiber，具体 transport 适配留在 Framework；默认入口通过标准 `http.Handler` 与 `server.RunHTTP` 管理 listener 和有界关闭。新增 `Proj/Other` 后可通过 `GO_PROJECT=Other` 使用同一套根命令。
...
Go 服务使用官方 OpenTelemetry SDK 创建 HTTP server、`project.get` application child 和低敏 Redis client span。OTLP/HTTP exporter 默认关闭，可通过 `Proj/Example/.env.example` 中的 `OTEL_*` 变量启用有界 batch、采样、导出 timeout 和 shutdown flush；HTTP attempt timeout 与 retry 退避均包含在总导出预算内，逐次 attempt 和最终 batch/span 只记录固定成功/失败结果。仓库已验证本地 wire contract、503 retry 恢复和持续故障上限，真实 collector、trace backend 和告警链路仍需目标环境证据。
...
| `yarn bench:transports` | 在 Linux 目标环境以 5 轮真实 TCP 对照 Fiber 与 `net/http`，覆盖固定并发和连接 churn 容量矩阵；非 Linux 平台跳过可比测量但仍运行 TCP 契约测试 |
...
| `yarn contracts:check` | 校验所有 `Proj/<项目>` 的 OpenAPI 仓库/ref/document/SDK 映射，默认离线 |
...
Linux transport workflow 固定 `steady-c1`、`steady-c16`、`steady-c64` 和 `connection-churn-c16` 四种 workload，采集 throughput、p50/p95/p99、连接获取、拨号、在途峰值、alloc/malloc、GC、goroutine、FD、进程 CPU/RSS 及前后网络/内存/socket 快照。`scripts/transport-benchmark-report.mjs` 只接受完整 5 轮、零错误和稳定 payload 的 4 × 2 矩阵，并生成中位数与方向比。
...
- `SDK/GoExample` 由该 OpenAPI 生成并保持相同版本；`support/consumer/HealthProbe` 使用 SDK 完成本地 readiness 弃用迁移演练，版本与证据边界见 `docs/openapi/consumer-matrix.md`。
...
- [待优化 V12（非 MSFront）](./docs/待优化/待优化V12.md)
